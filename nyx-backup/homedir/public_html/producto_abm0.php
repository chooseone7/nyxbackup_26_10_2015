...
$('#cambio_frm').blur(function() {
$('#item_nro').load('actualizar_producto.php',{usuario:$('#idFormulario').val()}); });
</script>

 <script type="text/javascript">
		function validarForm(){
		var verificar=true;
		if(!document.alta_producto.item_nro.value){
			alert("el campo item_nro es requerido");
			document.alta_producto.item_nro.focus();
			verificar=false;
			}else if(!document.alta_producto.item_nro.value){
			  alert("El campo item_nro no es válido");
			  document.alta_producto.item_nro.focus()
				verificar=false;
			}else if(!document.alta_producto.descripcion.value){
			alert("el campo descripcion requerido");
			document.alta_productos.descripcion.focus();
			verificar=false;
			}
			else if(!document.alta_producto.descripcion.value){
			  alert("El campo item_nro no es válido");
			  document.alta_producto.descripcion.focus()
				verificar=false;
			}
			...
			window.onload=function(){
				document.alta_producto.btn_insertar_producto.onclick=validarForm;
...
<script>
	function validarForm(){
		var verificar=true;
		if(!document.alta_producto.item_nro.value){
			alert("el campo Item nro es requerido");
			document.alta_producto.item_nro.focus();
			verificar=false;
			}else if(!document.alta_producto.item_nro.value){
			  alert("El campo item_nro no es válido");
...
<script>
$(document).on('touchstart click', '#btn_insertar_producto', function(event){
        event.stopPropagation();
        event.preventDefault();
        if(event.handled !== true) {

            // Do your magic here.
                validarForm();    //alert("entro al touch");
            event.handled = true;
        } else {
            return false;
        }
...
<form id="alta-producto"  name="alta_producto" method="post" enctype="multipart/form-data"  action="sql/altaproducto.php"  >
				<fieldset id="producto_abm_fieldset">
						...
						<div class="centrar-form-productos">
							<input type="button" name="btn_insertar_producto" id="btn_insertar_producto" value="Insertar"/>
						</div>
				</fieldset>
</form>
</div>
