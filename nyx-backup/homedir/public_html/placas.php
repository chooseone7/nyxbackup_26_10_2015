	<section id="contenido">
				<section id="principal-productos">
					...
					if(is_numeric($nro_pagina))
						...
						$inicio=0;
						echo $inicio;
						$consulta="SELECT p.cod_producto, p.item_nro, p.watt, p.chip, p.input_voltage, p.lamp_size, p.hrs, p.material, p.luminous, p.luminous_flux, p.cut_out, p.body_color, p.led_color, p.life_span, p.ip, p.guarantee, p.led_type_qty, p.frecuency_range, p.image1, p.descripcion, c.nombre_categoria, tp.nombre_tipo_producto FROM  producto p LEFT JOIN categoria c on p.cod_categoria=c.cod_categoria LEFT JOIN tipo_producto tp on p.cod_tipo_producto=tp.cod_tipo_producto WHERE c.nombre_categoria= 'placas' ORDER BY watt LIMIT $inicio, $reg_por_pagina";
					...
					while($registro=$ejecutar_consulta->fetch_assoc()){
							  ...
							  echo "<a href='?op=descripcion_producto&item=".($item_nro)."'><img src='img/imgproductos/".$registro["image1"]."'align='middle'  width='200' height='120' /></a>";
							//	echo "<h3>Descripcion ".$registro['descripcion']."</h3>";
								...
								echo "<hr id='linea-producto'>";
										...
										echo "<div id='paginador'>";
										 if($nro_pagina>1)
											echo "<a href='?op=placas&num=".($nro_pagina-1)."'><img class='anterior'src='img/gif/arrow-left.gif' /></a>";
											for($i=1;$i<=$can_paginas;$i++){
												if($i==$nro_pagina)
												echo "<p style='font-size:1.3em;'>$i|</p> ";
												else
												echo "<a href='?op=placas&num=$i'> $i </a>";
											}
										if($nro_pagina<$can_paginas)
											echo "<a href='?op=placas&num=".($nro_pagina+1)."'><img class='siguiente' src='img/gif/arrow-right.gif'/></a>";
										echo "</div>";
					?>
