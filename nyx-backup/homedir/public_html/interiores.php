	<section id="contenido">
				<section id="principal-productos">
					...
					if(is_numeric($nro_pagina))
						...
						$consulta="SELECT * FROM producto ORDER BY watt LIMIT $inicio, $reg_por_pagina";
						//var_dump($consulta);exit;
						$can_paginas=ceil($nro_reg/$reg_por_pagina);
						$ejecutar_consulta=$conexion->query($consulta);
					$item_nro=$_GET["item"];
					  $categoria_pagina=$_GET["categoria_pagina"];
					  /*Agregado mas reciente 09/05/2015 sabado junto con &categoria_pagina*/
					while($registro=$ejecutar_consulta->fetch_assoc()){
							  $item_nro=$registro["item_nro"];
							  echo "<a href='?op=descripcion_producto&item=".($item_nro)."&categoria_pagina=interiores'><img src='img/imgproductos/interiores/".$registro["image1"]."'align='middle'  width='200' height='120' /></a>";
							//	echo "<h3>Descripcion ".$registro['descripcion']."</h3>";
								...
								echo "<hr id='linea-producto'>";
										...
										echo "<div id='paginador'>";
										 if($nro_pagina>1)
											echo "<a href='?op=interiores&num=".($nro_pagina-1)."'><img class='anterior'src='img/gif/arrow-left.gif' /></a>";
											for($i=1;$i<=$can_paginas;$i++){
												if($i==$nro_pagina)
												echo "<p style='font-size:1.3em;'>$i|</p> ";
												else
												echo "<a href='?op=interiores&num=$i'> $i </a>";
											}
										if($nro_pagina<$can_paginas)
											echo "<a href='?op=interiores&num=".($nro_pagina+1)."'><img class='siguiente' src='img/gif/arrow-right.gif'/></a>";
										echo "</div>";
					?>
				</div>
